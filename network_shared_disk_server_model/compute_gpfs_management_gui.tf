...
resource "oci_core_instance" "mgmt_gui_node" {
  count               = var.mgmt_gui_node_count
  availability_domain = local.ad

  fault_domain        = "FAULT-DOMAIN-${(count.index%3)+1}"
  compartment_id      = var.compartment_ocid
  display_name        = "${var.mgmt_gui_node_hostname_prefix}${format("%01d", count.index+1)}"
  shape               = var.mgmt_gui_node_shape

  create_vnic_details {
    subnet_id           = local.client_subnet_id
    hostname_label      = "${var.mgmt_gui_node_hostname_prefix}${format("%01d", count.index+1)}"
...
resource "null_resource" "deploy_ssh_keys_on_mgmt_gui_node" {
  depends_on = [
...
resource "null_resource" "configure_mgmt_gui_service" {
  ...
  count = var.mgmt_gui_node_count > 0 ? 1 : 0
  # 1
  triggers = {
    instance_ids = "oci_core_instance.mgmt_gui_node.*.id"
  }
