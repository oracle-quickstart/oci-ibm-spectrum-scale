...
data "oci_core_instance_credentials" "InstanceCredentials" {
  ...
  instance_id = (var.windows_smb_client["node_count"] > 0 ? element(concat(oci_core_instance.windows_smb_client.*.id, [""]), 0) : "")
}
...
value = <<END
   ...
   InstancePublicIP: ${var.windows_smb_client["node_count"] > 0 ? oci_core_instance.windows_smb_client.*.public_ip[0] : ""}
   InstancePrivateIP: ${var.windows_smb_client["node_count"] > 0 ? oci_core_instance.windows_smb_client.*.private_ip[0] : ""}
END
}
