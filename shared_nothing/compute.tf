...
resource "null_resource" "deploy_ibm_ss" {
    depends_on = ["oci_core_instance.ServerNode" , "oci_core_volume.ServerNode_blockvolume", "oci_core_volume_attachment.ServerNode_blockvolume_attach" ]
    count               = "1"
    triggers {
 ...
 */
    provisioner "file" {
      source = "../shared_nothing_scripts/nodes-cloud-init-complete-status-check.sh"
    ...
    provisioner "file" {
      source = "../shared_nothing_scripts/deploy_ibm_ss.sh"
